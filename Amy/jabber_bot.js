...
controller.hears(['Hi','hi there','Hi Amy','Hello','Hey','Hiya'], ['direct_mention', 'self_message', 'direct_message'], function (bot, message) {
    let reply_message = {};
    let to = message.user;
    let type = message.group ? 'groupchat' : 'chat';

    let body = 'Hello Kaifee';
    reply_message.text = body;
    reply_message.stanza = xml`<message to="${to}" type="${type}"><body>${body}</body></message>`;
    bot.reply(message, reply_message);
});

controller.hears(['how are you','hru','hows you','how are you doing today?'], ['direct_mention', 'self_message', 'direct_message'], function (bot, message) {
    let reply_message = {};
    let to = message.user;
    let type = message.group ? 'groupchat' : 'chat';

    let body = `I'm good, thanks, how are you today?`;
    reply_message.text = body;
    reply_message.stanza = xml`<message to="${to}" type="${type}"><body>${body}</body></message>`;
    bot.reply(message, reply_message);
});

controller.hears(['I need some help', 'I need help', 'can you help?'], ['direct_mention', 'self_message', 'direct_message'], function (bot, message) {
    let reply_message = {};
    let to = message.user;
    let type = message.group ? 'groupchat' : 'chat';

    let body = `sure, what do you need help with?`;
    reply_message.text = body;
    reply_message.stanza = xml`<message to="${to}" type="${type}"><body>${body}</body></message>`;
    bot.reply(message, reply_message);
});

controller.hears(['issue*'], ['direct_mention', 'self_message', 'direct_message'], function (bot, message) {
    let reply_message = {};
    let to = message.user;
    let type = message.group ? 'groupchat' : 'chat';

    let body = `ok, let me raise it with the respective team and they will get back to you soon!`;
    reply_message.text = body;
    reply_message.stanza = xml`<message to="${to}" type="${type}"><body>${body}</body></message>`;
    bot.reply(message, reply_message);
});
